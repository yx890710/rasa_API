...
zh_rasa_chat_url = "http://{}:5005/model/parse".format(rasa_chat_host)
...
zh_nlu_model=new_report(zh_latest)
zh_nlu_model=zh_nlu_model.replace("\\","/")

#zh_nlu_model="./rasa_zh/models/v2021-08-21_15-42-57/nlu-20210821-154555.tar.gz"
#==Agent建置==
...
def rasa_run_zh_model():
    ...
    print("nlu_model")
    print(zh_nlu_model)
    zh_cmd="python -m rasa run -m {} --enable-api --log-file oo_zh.log -p 5005 -vv".format(zh_nlu_model)
    #使用shlex.split()拆解指令列
    ...
    # 因此會造成子進程無法被刪除
    subp = subprocess.Popen(zh_args,shell=False,encoding="utf-8")
...
def db_to_rasa_data():
    ...
    result_json=json.dumps({"rasa_zh":r1})#,"rasa_en":r2
    return result_json
...
def rasa_nlu_train():
    ...
    with open(config_name+'.yml','r',encoding="utf-8") as ttfile:
        data_loaded = yaml.safe_load(ttfile)

    data_loaded["pipeline"][0]["dictionary_path"]=dataset+"jieba_userdict" #修改位置
    #直接覆寫檔案
    with open(config_name+'.yml', 'w',encoding="utf-8") as f:
...
def model_change():
    ...
    zh_nlu_model=new_report(model_list[0])
    zh_nlu_model=zh_nlu_model.replace("\\","/")
    ...
    try:
        ...
        #windows殺死舊模型副程式
        subprocess.call(['taskkill', '/F', '/T', '/PID',  str(zh_process_pid)])
        zh_cmd="rasa run -m {} --enable-api --log-file oo_zh_1.log -p 5005 -vv".format(zh_nlu_model)
        zh_args = shlex.split(zh_cmd)
        print(zh_args)
        #建立新模型server子進程
        subp=subprocess.Popen(zh_args,shell=False,encoding="utf-8")
